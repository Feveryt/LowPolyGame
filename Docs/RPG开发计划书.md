...
- 基础数值使用 `CharacterStatsDefinition : ScriptableObject`：只保存 ID、生命、攻击、防御、体力、蓝量及体力恢复参数；当前资源值始终保存在角色运行时组件中。
- 首版伤害公式为 `Round(攻击 × 倍率 × 100 / (100 + 防御))`，正伤害最少为 `1`；暴击、元素、穿透、格挡和随机浮动在后续机制出现时再扩展。
- 当前内容规模不使用 Excel 转 JSON 与 `JsonUtility`。当敌人、物品或技能条目增长到需要批量调表时，保持运行时接口不变，再迁移为数据表导出管线。
